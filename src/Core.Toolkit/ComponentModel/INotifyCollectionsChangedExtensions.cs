namespace Core.ComponentModel
{
    ...
    {
        public static void RegisterAddAndRemoveDelegates<T>(
            this INotifyCollectionChanged collection, Action<T> addMethod, Action<T> removeMethod) where T : class
        {
        ...
        private static void DoForEachItemInList<T>(IList list, Action<T> action) where T : class
        {
